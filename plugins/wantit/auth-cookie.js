...
exports.register.attributes = {
  name: 'WantitAuthCookie',
};

exports.options = internals.options = {
  cacheOptions: { segment: 'wantit-sessions', expiresIn: 3 * 24 * 60 * 60 * 1000 },
  password: process.env.COOKIE_SECRET,
  cookie: 'wantit-sid',
  redirectTo: '/wantit/login',
  isSecure: (process.env.NODE_ENV === 'production' || process.env.NODE_ENV === 'staging'),
  requestDecoratorName: 'cookieAuthWantit',
};
...
internals.after = (server, next) => {
  ...
  server.auth.strategy('wantit-session', 'cookie', false, {
